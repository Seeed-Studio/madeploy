...
ma_err_t Yolo11Pose::postProcessF32() {
    ...
    for (auto& bbox : multi_level_bboxes) {
        ...
        for (int i = 0; i < num_keypoints_; ++i) {
            n_keypoint[i].x = data[bbox.index + num_record_ * (5 + i * 3 + 0)] / img_.width;
            n_keypoint[i].y = data[bbox.index + num_record_ * (5 + i * 3 + 1)] / img_.height;
            n_keypoint[i].z = data[bbox.index + num_record_ * (5 + i * 3 + 2)];
        }
        ...
        results_.emplace_back(std::move(keypoint));
    }
